...
CAPACIDAD_MAX = 10
TARIFA_POR_HORA = 2.5  # Tarifa base por hora
...
autos_estacionados = []
# Diccionario para guardar las horas de entrada de cada auto
tiempos_entrada = {}
...
def mostrar_menu():
    ...
    table.add_column("Descripción", style="green" , justify='center')
...
def calcular_tarifa(hora_entrada: datetime, hora_salida: datetime) -> float:
    """
    Calcula la tarifa según el tiempo real estacionado.
    - cobra por hora completa
    - si hay fracción, se cobra proporcionalmente
    """
    if hora_salida < hora_entrada:
        raise ValueError("La hora de salida no puede ser anterior a la de entrada")
    
    duracion = hora_salida - hora_entrada  # timedelta
    horas = duracion.total_seconds() / 3600  # convierte a horas como float
    tarifa = round(horas * TARIFA_POR_HORA, 2)
    
    # Mostrar información del cálculo
    console.print(f"[cyan]Hora de entrada: {hora_entrada.strftime('%Y-%m-%d %H:%M:%S')}[/cyan]")
    console.print(f"[cyan]Hora de salida: {hora_salida.strftime('%Y-%m-%d %H:%M:%S')}[/cyan]")
    console.print(f"[cyan]Tiempo estacionado: {duracion}[/cyan]")
    console.print(f"[cyan]Horas: {horas:.2f}[/cyan]")
    console.print(f"[cyan]Tarifa calculada: ${tarifa}[/cyan]")
...
def simular_entrada():
    ...
    # Tomar tiempo de entrada del sistema automáticamente
    ...
    # Guardar la hora de entrada para este auto
    tiempos_entrada[placa] = hora_entrada
    historial.guardar_evento(placa, hora_entrada, "entrada")
    
    console.print(f"[green]Auto {placa} ha entrado. Hora: {hora_entrada.strftime('%Y-%m-%d %H:%M:%S')}[/green]")
...
def simular_salida():
    ...
    # Verificar que tengamos el tiempo de entrada
    if placa not in tiempos_entrada:
        console.print(f"[red]Error: No se encontró la hora de entrada para {placa}[/red]")
        return
    
    # Tomar tiempo de salida del sistema automáticamente
    hora_salida = datetime.now()
    hora_entrada = tiempos_entrada[placa]
    
    # Calcular tarifa automáticamente
    tarifa = calcular_tarifa(hora_entrada, hora_salida)
    
    # Actualizar estado
    parqueo.actualizar(parqueo.ocupados - 1)
    autos_estacionados.remove(placa)
    # Eliminar el tiempo de entrada ya que el auto salió
    del tiempos_entrada[placa]
    
    historial.guardar_evento(placa, hora_salida, "salida", tarifa)
    
    console.print(f"[yellow]Auto {placa} ha salido. Hora: {hora_salida.strftime('%Y-%m-%d %H:%M:%S')} | Tarifa: ${tarifa}[/yellow]")
    mostrar_estado_resumido()
    ...
    try:
        ...
        for i in range(min(salidas, len(autos_estacionados))):
            if autos_estacionados:
                ...
                # Tomar tiempo de salida del sistema
                hora_salida = datetime.now()
                
                # Calcular tarifa si tenemos tiempo de entrada
                if placa in tiempos_entrada:
                    hora_entrada = tiempos_entrada[placa]
                    tarifa = calcular_tarifa(hora_entrada, hora_salida)
                    del tiempos_entrada[placa]  # Eliminar el tiempo de entrada
                else:
                    # Si no hay tiempo de entrada, usar tarifa mínima por hora
        ...
        for i in range(min(entradas, espacios_disponibles)):
            ...
            # Tomar tiempo de entrada del sistema
            hora_entrada = datetime.now()
...
def ver_autos_estacionados():
    ...
    table.add_column("Hora de Entrada", style="yellow")
    
    for i, placa in enumerate(autos_estacionados, 1):
        hora_entrada_str = tiempos_entrada[placa].strftime('%Y-%m-%d %H:%M:%S') if placa in tiempos_entrada else "N/A"
        table.add_row(str(i), placa, "Estacionado", hora_entrada_str)
        ...
        for _ in range(salidas):
            if autos_estacionados:
                ...
                if placa in tiempos_entrada:
                    ...
                    # Si no hay tiempo de entrada, usar tarifa mínima
                    tarifa = TARIFA_POR_HORA
